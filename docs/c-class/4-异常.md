...
当`first_number = 'a'`或`second_number = 'a'`，程序还会出现异常。
...
断言（Assertion）是编程中，用于判断程序在特定执行点上，必须满足一定条件的语句。它是对程序状态的一种明确声明，如果该声明被违反，则表明程序存在错误。
