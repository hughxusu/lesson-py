...
<img src="./assets/class-instance.jpeg" style="zoom:55%;" />
...
* 在方法`show`中执行`self.xxx`，可以读取实例中的数据。
...
在Python中语言中，类定义时有一些具有特殊功能的属性和方法，如：`__init__`、`__eq__`

其他常用魔法属性

1. `__str__()`：当使用`print`输出对象的时候，默认打印对象的内存地址。如果类定义了该方法，那么就会打印从在这个方法中`return`的数据。
...
2. `__del__()`：当删除对象时，python解释器也会默认调用该方法。
3. `__dict__`
...
> [!warning]
>
> 魔法属性和方法虽然也以`__`开始，但并不是私有的，也可以直接调用。如果自定义属性或方法，如`__xx__`形式且不与魔法属性和魔法方法重名，不会变为私有，但是不推荐如此定义。
