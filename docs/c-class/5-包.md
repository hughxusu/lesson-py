...
借助模块和包，程序员可以将代码组织成多个文件，每个文件负责不同的功能。

## 模块的使用

一个Python文件可以看做一个模块，模块中能定义函数，类和变量。
...
1. `__all__`变量：模块级变量，是一个列表，定义在 Python 模块中，用于控制从模块导入时的行为。只有当使用`from import *`时，仅仅导入`__all__`中列出的名字。

2. 模块中的私有化。在包中使用`_xx`单前置下划线的变量、函数和类，无法使用`from xx import yy`。
