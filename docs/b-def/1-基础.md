...
<img src="./assets/Xnip2025-11-13_21-12-50.jpg" style="zoom:50%;" />

1. 定义函数时首先应该声明函数，Python使用`def`关键字来声明函数
2. 函数名由程序员指定，函数名命名规范
   * 和变量命名规则一致，使用蛇形命名法。
   * 前缀可以加动词，如：can、has、is、get、set、load等。
...
<img src="./assets/Xnip2025-11-13_21-48-53.jpg" style="zoom:50%;" />
...
1. 定义一个函数，将用户输入名称，拼接成字符串"Hello Eric, would you like to learn some Python today?"输出。
1. 定义一个函数，输入为n，返回一个值为从1到n的交错计算结果，如：n=5，result=-1+2-3+4-5。
