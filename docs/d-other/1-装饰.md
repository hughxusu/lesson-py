...
> [!important]
>
> 闭包的本质是一个函数与其相关的引用环境组合的一个整体，可以看做一个简写的类。
...
* `nonlocal count`声明的变量是外层定义的，而不是局部变量，作用类似于`global`。
...
使用Python的计时工具包，`time.time()`返回的是从 1970年1月1日00:00:00 UTC （Unix纪元）开始到当前时间的秒数 ，包括小数部分。
...
1. `@CountCalls`等价于`greet = CountCalls(greet)`，所以需要提供一个`init`方法，并增加`func`参数。
2. 要想类的实例对象能够像函数一样调用，需要在类里面实现`__call__`方法，把类的实例变成可调用对象(callable)，也就是说可以像调用函数一样进行调用。
3. 在`__call__`方法里进行对`func`函数的装饰，可以添加额外的功能。
...
增加删除属性，使用`@percent.deleter`装饰属性，当使用`del`执行删除时，会调用被装饰的方法。
...
使用`property()`函数可以，将类的方法伪装成普通属性来访问。
```python
xxx = property(fget=None, fset=None, fdel=None, doc=None)
```
...
1. 定义一个装饰器个网址前面添加'https://'。
1. 定义一个权限控制装饰器，使用该装饰器装饰函数时，根据输入用户参数的权限来判断函数是否可以被执行。
