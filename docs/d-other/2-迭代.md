...
1. 惰性求值：只在需要时计算值，不预先计算所有结果。
1. 一次性遍历：遍历后无法重置，需要重新创建。
...
<img src="./assets/2177978-20201018205525401-1392476113.png" style="zoom:90%;" />
...
1. 定义一个迭代器类，该类可以接收数据数组、元组和字符串，该迭代器可以正向无限循环输出元素，也可以切换迭代方向。
2. 定义一个生成器函数，以特定的滑动窗口长度，计算输入序列的平均值。
