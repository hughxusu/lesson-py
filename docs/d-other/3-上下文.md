...
上下文管理器是一种用于管理资源（如文件、数据库连接、锁等）的机制，它确保资源在使用后被正确清理和释放。Python中上下文管理器通过`with`语句使用。
...
`__enter__()`方法报错，则不会调用`__exit__()`方法
...
只要`__enter__()`调用成功，则一定会执行`__exit__()`方法
...
1. 定义一个字典操作上下文管理器，接受一个字典变量，返回一个操作字典，然后对字典进行操作，如果没有异常，字典操作成功，如果参数异常，则回滚操作，不改变字典中的数据。
