...
conda update numpy=1.93 # 指定包的版本
pip install robotframework==2.8.7 #pip指定版本

# 更新package
conda update -n python34 numpy
conda update --all # 升级全部包
...
# 安装多个包
conda install -c conda-forge django-crispy-forms django-reversion django-formtools xlwt xlsxwriter
```
...
1. 创建`docs`文件夹进入到文件夹下，执行`sphinx-quickstart`命令。
2. 配置conf.py文件
...
3. 生成自动Api文档，注意保证所目录下所有程序都是python包

```shell
# sphinx-apidoc -o [指定输出路径] [程序路径]

cd
```

4. 增加index.rst文件路径，模块之间需要有空行
...
6. 增加`md`文档，配置conf.py文件，安装recommonmark库
