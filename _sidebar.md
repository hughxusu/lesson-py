

* 引言
  * [认识Python](a-introduce/1-认识)
  * [搭建Python的开发环境](a-introduce/2-环境)
* Python语言
  * [从变量开始](b-basic/1-变量)
  * [数据类型](b-basic/2-类型)
  * [变量的基本操作](b-basic/3-操作)
  * [程序的执行原理](b-basic/4-原理)
  * [流程控制](c-flow/index)
    * [条件控制](c-flow/1-条件)
    * [循环控制](c-flow/2-循环)
  * [序列类型](d-type/index)
    * [字符串](d-type/1-字符串)
    * [列表](d-type/2-列表)
    * [元组](d-type/3-元组)
    * [字典](d-type/4-字典)
    * [集合](d-type/5-集合)
    * [相似操作](d-type/6-操作)
  * [函数](e-function/index)
    * [基本语法](e-function/1-基础)
    * [用户管理](e-function/2-用户)
    * [Python中的特殊函数](e-function/3-特殊)
    * [文件操作](e-function/4-文件)
  * [面向对象](f-class/index)
    * [基本语法](f-class/1-基础)
    * [面向对象的三大特性](f-class/2-特性)
    * [面向对象的其他特性](f-class/3-其它)
    * [异常、模块和包](f-class/4-异常)
    * [面向对象版学员管理系统](f-class/5-系统)
  * 其他高级特性
    * [装饰器](g-other/1-装饰)
    * [with语句与上下文管理器](g-other/2-上下文)
    * [生成器与拷贝](g-other/3-拷贝)

* 附录
  * [运算符](h-ps/1-运算符)

- **链接** 
[<img src="https://static.runoob.com/images/favicon.ico" alt="Code" style="zoom: 60%;" />菜鸟教程](https://www.runoob.com/)
[习题网站](https://pynative.com/python-exercises-with-solutions/)