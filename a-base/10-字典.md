...
<img src="https://raw.githubusercontent.com/hughxusu/lesson-py/developing/_images/base/Dictionary-Key-Value-Pairs-Illustration.png" style="zoom:85%;" />
...
4. `items()`返回一个键值对组成的可迭代对象，每个键值对是一个元组。
...
1. `add(obj)`向集合内追加数据，如果集合中存在该数据，则不进行任何操作。集合中数据排序与添加顺序无关。
...
2. `update(sqe)`向集合中追加序列，如果`sqe`的类型不是序列类型会报错。
