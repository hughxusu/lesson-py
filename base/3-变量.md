...
在PyCharm中创建一个`main.py`文件，在文件中输入如下文本，并执行程序。
...
**标识符**：就是程序员定义的变量名、函数名等。标识符定义时应该做到见名知义。
...
2. `NameError: name 'mesage' is not defined`解释器发现了一名称错误，变量`mesage`没有定义

指程序不能正常执行，或执行结果不符合预期，俗称Bug。产生的原因：
...
Python官方提供有一系列PEP（Python Enhancement Proposals）文档，其中第8篇文档专门针对Python的代码格式给出了建议，俗称**PEP8**。
